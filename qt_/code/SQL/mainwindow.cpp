...
void MainWindow::refreshList()
{
    ...
    for (int i = 0; i < ui->output->count(); ++i)
        list->append(ui->output->item(i)->text());
}
...
void MainWindow::on_search_for_textChanged(const QString &arg1)
{
    QRegExp qregexp(arg1, Qt::CaseSensitive, QRegExp::Wildcard);
    ui->output->clear();
    ui->output->addItems(list->filter(qregexp));
}
